...
export const MessMenuCard = () => {
	...
	const EmptyData = () => <Type style={{ marginTop: "0" }}>No data available.</Type>;
	const EmptyMenu = () => <Type style={{ marginTop: "0" }}>Not yet updated.</Type>;
	const LoadingData = () => <Type>Loading...</Type>;
	const MealText = ({ meal }) => <MenuItem>{meal.menu.map((item, i) => (i < meal.menu.length - 1 ? item + ", " : item))}</MenuItem>;
	...
	const renderMeal = (meal, i, total) => (
		<div key={i.toString()}>
			<MealContainer>
				...
				<SpecialPill>{meal.isSpecial ? <Caption>✨ SPECIAL MEAL ✨</Caption> : null}</SpecialPill>
			</MealContainer>
